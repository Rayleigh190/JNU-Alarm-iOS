...
extension Bundle {
    ...
    var QUESTION_API_URL: String? {
        guard let file = self.path(forResource: "Secrets", ofType: "plist") else{return ""}
        guard let resource = NSDictionary(contentsOfFile: file) else { return "" }
        guard let key = resource["QUESTION_API_URL"] as? String else {fatalError("QUESTION_API_URL error")}
        return key
    }
    
    var APP_INFO_API_URL: String? {
        guard let file = self.path(forResource: "Secrets", ofType: "plist") else{return ""}
        guard let resource = NSDictionary(contentsOfFile: file) else { return "" }
        guard let key = resource["APP_INFO_API_URL"] as? String else {fatalError("APP_INFO_API_URL error")}
        return key
    }
